...
{
    private static final long serialVersionUID = 4007698055412556291L;
...
//    @SuppressWarnings("unchecked")
    ...
    private void initComponents() {
        ...
        animationList = new JListMutable<Animation>(new DefaultMutableListModel<Animation>());
        ...
        animationList.setModel(new DefaultMutableListModel<Animation>());
        ((JListMutable<Animation>)animationList).setListCellEditor(new DefaultListCellEditor<Animation>(new JTextField()));
        ...
        spriteList.setModel(new DefaultListModel<GraphicObject>());
    ...
    private ArrayList<GraphicObject> addNodeToCell(CustomNode aNode, AnimationCell aCell, ArrayList<GraphicObject> aGraphics, Point aPoint)
    {
        ...
        if (aNode.isLeaf())
        {
            if (aCell != null)
            {
                ...
                ((DefaultListModel<GraphicObject>)spriteList.getModel()).addElement(graphic);
    ...
    {
        DefaultMutableListModel<Animation> model = (DefaultMutableListModel<Animation>)animationList.getModel();
        int index = animationList.getSelectedIndex();
        if (index >= 0)
        {
...
private void zOrderSpinnerStateChanged(javax.swing.event.ChangeEvent evt) {//GEN-FIRST:event_zOrderSpinnerStateChanged
    ...
    private void populateSpriteListFromCell(AnimationCell aCell)
    {
        ...
        DefaultListModel<GraphicObject> model = (DefaultListModel<GraphicObject>)spriteList.getModel();
        model.clear();
...
private void exportGifButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_exportGifButtonActionPerformed
    ...
    {    
        ...
        {
            if (((DefaultListModel<GraphicObject>)spriteList.getModel()).elementAt(i) == aGraphic)
                bFound = true;
        }
        if (!bFound)
            ((DefaultListModel<GraphicObject>)spriteList.getModel()).addElement(aGraphic);
    ...
    public void graphicErased(GraphicPanel aPanel, GraphicObject aGraphic)
    {
        ...
        workingCell.rebuild();
        ((DefaultListModel<GraphicObject>)spriteList.getModel()).removeElement(aGraphic);
        animationStripPanel.repaint();        
    }
    ...
    public void valueChanged(ListSelectionEvent e)
    {
        ...
        {
//            JList<Animation> list = (JList<Animation>)e.getSource();
            Object source = e.getSource();
                                    
//            int selectedIndices[] = list.getSelectedIndices();

            if (source == animationList)
            {
                ...
                boolean bEnabled = (! ((DefaultMutableListModel<Animation>)animationList.getModel()).isEmpty()) && (animationList.getSelectedIndex() >= 0);
                removeAnimationButton.setEnabled(bEnabled);
                duplicateAnimationButton.setEnabled(bEnabled);
                exportGifButton.setEnabled(bEnabled);
            } 
            else if (source == spriteList)
            {
                int selectedIndices[] = spriteList.getSelectedIndices();
    ...
    {
        private static final long serialVersionUID = 8100919723439871962L;
    ...
    {
        private static final long serialVersionUID = -8284278922128586385L;
    ...
    private javax.swing.JList<Animation> animationList;
